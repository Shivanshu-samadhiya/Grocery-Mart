...
const DeliveryLayout = () => {
  ...
  const menuItems = [
    { path: '/delivery/dashboard', name: 'Dashboard', icon: 'bi-speedometer2' },
    { path: '/delivery/orders', name: 'My Orders', icon: 'bi-receipt' },
    { path: '/delivery/history', name: 'History', icon: 'bi-clock-history' },
    { path: '/delivery/profile', name: 'Profile', icon: 'bi-person-gear' }
  ];

  const getPageTitle = () => {
    switch (activePath) {
      case '/delivery/dashboard': return 'Delivery Dashboard';
      case '/delivery/orders': return 'My Active Orders';
      case '/delivery/history': return 'Delivery History';
      case '/delivery/profile': return 'My Profile';
    ...
    <div className="admin-shell">
      ...
      <div className={`admin-sidebar ${showMobileSidebar ? 'show' : ''}`}>
        <div className="admin-sidebar-logo d-flex align-items-center justify-content-between">
          <Link className="d-flex align-items-center text-decoration-none" to="/delivery/dashboard">
            <img 
              src={logo} 
            ...
            <span className="fw-extrabold text-success fs-5" style={{ letterSpacing: '-0.5px' }}>
              🛒 Grocery<span className="text-warning">Mart</span> <span className="text-dark small fs-6">Delivery</span>
            </span>
          </Link>
        ...
        <div className="p-3 border-top bg-light">
          <div className="small text-muted mb-1">Delivery Agent:</div>
          <div className="fw-bold text-truncate small mb-3 text-dark">{user?.username || 'Delivery Agent'}</div>
          <button 
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div className="d-flex align-items-center gap-2">
              <div className="bg-success text-white rounded-circle d-flex align-items-center justify-content-center fw-bold" style={{ width: '36px', height: '36px', fontSize: '0.9rem' }}>
                {user?.username?.substring(0, 2).toUpperCase() || 'DL'}
              </div>
              <div className="d-none d-lg-block text-start">
                <div className="fw-bold text-dark small m-0" style={{ lineHeight: '1.2' }}>{user?.username || 'Delivery Agent'}</div>
                <span className="text-muted text-uppercase" style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.5px' }}>Delivery Partner</span>
              </div>
            </div>
          </div>
        </header>
